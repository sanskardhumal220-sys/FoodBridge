...
import { HelpCircle, Mail, Bug, MessageSquare, Book, Phone, MapPin, Clock, AlertOctagon, User, Heart, Truck, Building, Shield, ShieldCheck, Download } from 'lucide-react';
  ...
  return <div className="min-h-screen bg-gray-50 dark:bg-gray-950 py-12 lg:py-24 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        y: 0
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-4">{t("support.text1")}</h1>
          <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">{t("support.text2")}</p>
        </motion.div>

        {/* Main Support Hub Cards */}
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-16">
          <HubCard to="/faq" icon={<HelpCircle />} title={t("support.attr3")} desc="Find common answers" color="text-blue-500" bg="bg-blue-100 dark:bg-blue-900/30" />
          <HubCard to="/contact" icon={<Mail />} title={t("support.attr4")} desc="Reach our team" color="text-green-500" bg="bg-green-100 dark:bg-green-900/30" />
          <HubCard to="/report-issue" icon={<Bug />} title={t("support.attr5")} desc="Tell us about a bug" color="text-red-500" bg="bg-red-100 dark:bg-red-900/30" />
          <HubCard to="/feedback" icon={<MessageSquare />} title={t("support.attr6")} desc="Share your thoughts" color="text-orange-500" bg="bg-orange-100 dark:bg-orange-900/30" />
          <HubCard to="/faq" icon={<Book />} title={t("support.attr7")} desc="Read our guides" color="text-purple-500" bg="bg-purple-100 dark:bg-purple-900/30" />
        </motion.div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-16">
          ...
          delay: 0.2
            ...
            <div className="flex-grow text-center md:text-left">
              <div className="flex items-center justify-center md:justify-start gap-2 mb-2">
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{t("support.text8")}</h3>
                <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400">
                  <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />{t("support.text9")}</span>
              </div>
              <p className="text-gray-600 dark:text-gray-400 mb-4">{t("support.text10")}<span className="font-semibold text-gray-900 dark:text-white">{t("support.text11")}</span></p>
              <button onClick={() => triggerNotification('Connecting to a live agent...')} className="px-8 py-3 bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-bold rounded-xl hover:scale-105 transition-transform shadow-md">{t("support.text12")}</button>
            </div>
          </motion.div>
          ...
          delay: 0.3
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white border-b border-gray-100 dark:border-gray-800 pb-4">{t("support.text13")}</h3>
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          delay: 0.4
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">{t("support.text14")}</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {HELP_CATEGORIES.map((cat, idx) => <Link key={idx} to="/faq" className="flex flex-col items-center justify-center p-6 rounded-2xl bg-white dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700 hover:border-primary-400 transition-colors text-center group">
          ...
          delay: 0.5
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white">{t("support.text15")}</h3>
            <div className="space-y-3">
              ...
              <button onClick={() => triggerNotification('User Guide downloaded successfully!')} className="w-full flex items-center gap-3 p-3 rounded-xl bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-400 hover:bg-primary-100 transition-colors font-medium text-sm text-left">
                <Download className="w-5 h-5 shrink-0" />{t("support.text16")}</button>
            </div>

            <div className="pt-6 border-t border-gray-100 dark:border-gray-800">
              <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-4 text-center">{t("support.text17")}</p>
