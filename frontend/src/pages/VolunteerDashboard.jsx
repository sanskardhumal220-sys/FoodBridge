...
const VolunteerDashboard = () => {
  ...
  useEffect(() => {
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        (pos) => fetchDonations(userInfo, pos.coords.latitude, pos.coords.longitude),
  ...
  const fetchDonations = async (currentUser, lat = null, lng = null) => {
    try {
      const queryParams = new URLSearchParams({ status: 'Accepted' });
      if (lat && lng) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {loading ? (
        <p className="dark:text-white text-center py-10">{t('volunteer_dashboard.loading')}</p>
      ) : (
        <div className="space-y-12">
          ...
          <div>
            ...
            {myDeliveries.length === 0 ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {myDeliveries.map((donation, index) => (
                  <DeliveryCard 
                    key={donation._id} 
                    donation={donation} 
                    index={index} 
                    actionText={t('volunteer_dashboard.mark_delivered')}
                    actionIcon={<CheckCircle2 size={20} />}
                    onAction={() => markDelivered(donation._id)}
                    onChat={() => openChat(donation._id)}
                    onTrack={() => setTrackingDonation(donation)}
                    btnClass="bg-green-500 hover:bg-green-600 shadow-lg shadow-green-500/30"
                  />
                ))}
              </div>
            )}
          ...
          <div>
            ...
            {availableDeliveries.length === 0 ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {availableDeliveries.map((donation, index) => (
                  <DeliveryCard 
                    key={donation._id} 
                    donation={donation} 
                    index={index} 
                    actionText="Claim Pickup"
                    actionIcon={<CheckCircle2 size={20} />}
                    onAction={() => claimDelivery(donation._id)}
                    btnClass="bg-primary-500 hover:bg-primary-600 shadow-lg shadow-primary-500/30"
                  />
                ))}
              </div>
            )}
...
const DeliveryCard = ({ donation, index, actionText, actionIcon, onAction, onChat, btnClass, onTrack }) => {
  const { t } = useTranslation();
  return (
  <motion.div 
    ...
    <div className="p-6 flex-grow flex flex-col">
      ...
      <div className="space-y-3 mb-6 flex-grow">
        ...
        <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
          ...
          <span>{t('volunteer_dashboard.feeds')} {donation.foodBrainData?.peopleFed || parseInt(donation.quantity) || 5} {t('volunteer_dashboard.people')} (Qty: {donation.quantity})</span>
        </div>
        <div className="flex items-center text-sm text-red-500 dark:text-red-400 font-medium">
          <Clock size={16} className="mr-2" />
          <span>{t('volunteer_dashboard.deliver_before')} {new Date(donation.expiryTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        {onTrack && (
          ...
          >
            <Navigation size={20} /> Start Live Tracking
          </button>
        )}

        <button 
          onClick={onAction}
          className={`w-full text-white py-3 rounded-xl font-semibold transition-colors flex items-center justify-center gap-2 ${btnClass}`}
        >
