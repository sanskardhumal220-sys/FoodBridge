...
const NGODashboard = () => {
  ...
  useEffect(() => {
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        (pos) => fetchData(user, pos.coords.latitude, pos.coords.longitude),
        () => fetchData(user)
      );
    } else {
      fetchData(user);
  ...
  const mapMarkers = donations.map(d => ({
    ...
    popupContent: (
      <div className="mt-2 space-y-2">
        ...
        <p className="text-xs flex items-center text-gray-700 font-semibold">Quantity: {d.quantity}</p>
        <p className="text-xs flex items-center text-red-500 font-bold"><Clock size={12} className="mr-1"/> {t('ngo_dashboard.expires')} {new Date(d.expiryTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 p-1 rounded-xl flex shadow-sm border border-gray-200 dark:border-gray-700">
          <button 
            ...
            className={`px-6 py-2 rounded-lg font-medium transition-all ${activeTab === 'available' ? 'bg-primary-500 text-white shadow-md' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'}`}
          >
            {t('ngo_dashboard.tab_available')}
          </button>
          <button 
            onClick={() => setActiveTab('rescues')}
            className={`px-6 py-2 rounded-lg font-medium transition-all ${activeTab === 'rescues' ? 'bg-primary-500 text-white shadow-md' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'}`}
          >
        ...
        <div className="flex justify-end mb-4">
          <div className="bg-white dark:bg-gray-800 p-1 rounded-lg flex shadow-sm border border-gray-200 dark:border-gray-700">
            <button 
              ...
              className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${viewMode === 'list' ? 'bg-gray-100 dark:bg-gray-700 text-primary-600 dark:text-primary-400' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
            >
              <List size={16} /> {t('ngo_dashboard.list_view')}
            </button>
            <button 
              onClick={() => setViewMode('map')}
              className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${viewMode === 'map' ? 'bg-gray-100 dark:bg-gray-700 text-primary-600 dark:text-primary-400' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
            >
      ...
      {loading ? (
        <p className="dark:text-white text-center py-10">{t('ngo_dashboard.loading')}</p>
      ) : activeTab === 'available' ? (
        // Available Tab
        donations.length === 0 ? (
          ...
          viewMode === 'map' ? (
            <RadarMapComponent markers={mapMarkers} className="h-[600px]" />
          ) : (
              ...
              {donations.map((donation, index) => {
                ...
                >
                  <div className="relative">
                    ...
                    {urgent && (
                      <div className="absolute top-4 right-4 bg-red-500 text-white px-3 py-1 rounded-full text-sm font-bold flex items-center gap-1 shadow-lg shadow-red-500/50 animate-pulse">
                  ...
                  <div className="p-6 flex-grow flex flex-col">
                    ...
                    <div className="space-y-3 mb-6 flex-grow">
                      <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <span>{t('ngo_dashboard.feeds')} {donation.foodBrainData?.peopleFed || parseInt(donation.quantity) || 5} {t('ngo_dashboard.people')} (Qty: {donation.quantity})</span>
                      </div>
                      <div className={`flex items-center text-sm font-medium ${urgent ? 'text-red-500 dark:text-red-400' : 'text-gray-600 dark:text-gray-400'}`}>
                        ...
                        <span>{t('ngo_dashboard.expires')} {new Date(donation.expiryTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                      </div>
                      <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <button 
                      ...
                      className={`w-full text-white py-3 rounded-xl font-semibold transition-colors flex items-center justify-center gap-2 ${urgent ? 'bg-red-500 hover:bg-red-600 shadow-lg shadow-red-500/30' : 'bg-primary-500 hover:bg-primary-600'}`}
                    >
              ...
              >
                <div className="relative">
                  ...
                  <div className="absolute top-4 right-4 bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm px-3 py-1 rounded-full text-sm font-bold shadow-md capitalize">
                    {donation.status === 'PickedUp' ? 'Volunteer on the way 🚚' : 
                    ...
                    <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                      ...
                      <span>
                        ...
                        {donation.distance !== undefined && (
                          <span className="ml-2 font-semibold text-primary-600 dark:text-primary-400">({donation.distance.toFixed(1)} km away)</span>
                        )}
                      </span>
                    </div>
                  </div>
                  ...
                  {donation.status === 'PickedUp' && (
                    <div className="flex flex-col gap-2 mt-3">
                      <button 
                        ...
                        className="w-full bg-blue-500 hover:bg-blue-600 text-white py-3 rounded-xl font-semibold transition-colors flex items-center justify-center gap-2 shadow-lg shadow-blue-500/30"
                      >
                        <Navigation size={20} /> Track Live Delivery
                      </button>
                      <button 
                        onClick={() => verifyDelivery(donation._id)}
                        className="w-full bg-primary-500 hover:bg-primary-600 text-white py-3 rounded-xl font-semibold transition-colors flex items-center justify-center gap-2 shadow-lg shadow-primary-500/30"
                      >
                        <Check size={20} /> Verify Delivery
                      </button>
                    </div>
                  )}
                </div>
