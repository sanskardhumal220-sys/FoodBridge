...
const DonorDashboard = () => {
  ...
  const [userInfo, setUserInfo] = useState({ name: 'Demo Donor', role: 'Donor' });
  ...
  // Nearby NGOs State
  const [donorLocation, setDonorLocation] = useState({ lat: 26.2183, lng: 78.1828 }); // Default: Gwalior, MP
  const [recommendedNgos, setRecommendedNgos] = useState([]);
  const [selectedNgo, setSelectedNgo] = useState(null);

  useEffect(() => {
    const user = JSON.parse(localStorage.getItem('userInfo')) || { name: 'Demo Donor', role: 'Donor' };
    setUserInfo(user);
  ...
  const ngoMarkers = recommendedNgos.map(ngo => ({
    ...
    popupContent: (
      <button className="w-full bg-primary-500 text-white py-1 rounded text-xs font-bold mt-2 hover:bg-primary-600 transition-colors" onClick={() => { setSelectedNgo(ngo); setActiveTab('create'); }}>
        Donate Here
      </button>
    )
  }));
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 p-1 rounded-xl flex shadow-sm border border-gray-200 dark:border-gray-700 overflow-x-auto">
          <button 
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-all whitespace-nowrap ${activeTab === 'create' ? 'bg-primary-500 text-white shadow-md' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'}`}
          >
            {t('donor_dashboard.tab_create')}
          </button>
          <button 
            onClick={() => setActiveTab('my_donations')}
            className={`px-4 py-2 rounded-lg font-medium transition-all whitespace-nowrap ${activeTab === 'my_donations' ? 'bg-primary-500 text-white shadow-md' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'}`}
          >
            {t('donor_dashboard.tab_my_donations')}
          </button>
          <button 
            onClick={() => setActiveTab('nearby_ngos')}
            className={`px-4 py-2 rounded-lg font-medium transition-all whitespace-nowrap ${activeTab === 'nearby_ngos' ? 'bg-primary-500 text-white shadow-md' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'}`}
          >
      ...
      {activeTab === 'create' && (
        <div className="max-w-4xl mx-auto">
          {step === 1 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass p-8 rounded-3xl">
              ...
              <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-2xl p-12 text-center hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors cursor-pointer relative">
                ...
                />
                {formData.image ? (
                  <img src={formData.image} alt="Preview" className="mx-auto max-h-64 rounded-lg" />
                ) : (
          ...
          {step === 2 && (
            <motion.form initial={{ opacity: 0 }} animate={{ opacity: 1 }} onSubmit={submitDonation} className="glass p-8 rounded-3xl space-y-6">
              
              {selectedNgo && (
                <div className="bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 border border-emerald-200 dark:border-emerald-800 p-4 rounded-2xl flex items-center justify-between mb-4">
                  <div>
                    <p className="text-sm font-semibold text-emerald-800 dark:text-emerald-300">Direct Donation Targeted</p>
                    <h4 className="text-lg font-bold text-emerald-900 dark:text-emerald-100">{selectedNgo.name}</h4>
                  </div>
                  <button type="button" onClick={() => setSelectedNgo(null)} className="text-xs text-emerald-600 hover:text-emerald-700 bg-emerald-100/50 px-3 py-1.5 rounded-lg font-semibold">
                    Clear target
                  </button>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input type="datetime-local" required onChange={e => setFormData({...formData, prepTime: e.target.value})} className="w-full px-4 py-3 rounded-xl border dark:border-gray-700 dark:bg-gray-800 dark:text-white" />
                </div>
                <div>
                  <label className="block text-sm text-gray-700 dark:text-gray-300 mb-2">{t('donor_dashboard.expiry_time')}</label>
                  <input type="datetime-local" required onChange={e => setFormData({...formData, expiryTime: e.target.value})} className="w-full px-4 py-3 rounded-xl border dark:border-gray-700 dark:bg-gray-800 dark:text-white" />
                </div>
              </div>
              <div>
              ...
              <button 
                ...
                className="w-full bg-primary-500 flex items-center justify-center gap-2 hover:bg-primary-600 disabled:bg-primary-300 text-white py-3 rounded-xl font-semibold transition-colors"
              >
              ...
              {myDonations.map((donation, index) => (
                ...
                >
                  <div className="relative">
                    {donation.image ? (
                      <img src={donation.image} alt={donation.foodType} className="w-full h-48 object-cover" />
                    ) : (
                  ...
                  <div className="p-6 flex-grow flex flex-col">
                    ...
                    <div className="space-y-2 mb-6 flex-grow">
                      <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <span>{t('donor_dashboard.expires')} {new Date(donation.expiryTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                      </div>
                      {(donation.status === 'accepted' || donation.status === 'picked_up') && (
                        <div className="flex items-center text-sm font-medium text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/30 p-2 rounded-lg mt-2">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="flex-1 glass p-6 rounded-3xl flex flex-col h-[700px]">
            <div className="flex justify-between items-center mb-4">
              ...
              <button 
                ...
                className="bg-blue-100 text-blue-700 hover:bg-blue-200 dark:bg-blue-900/30 dark:text-blue-400 px-4 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 transition-colors"
              >
          ...
          <div className="lg:w-96 glass p-6 rounded-3xl h-[700px] flex flex-col overflow-hidden">
            <h3 className="text-xl font-bold dark:text-white mb-4 flex items-center gap-2">
              <Star className="text-accent-500" /> Recommended NGOs
            </h3>
            <div className="flex-1 overflow-y-auto pr-2 space-y-4 custom-scrollbar">
              {recommendedNgos.map((ngo, idx) => (
                <motion.div 
                  ...
                  key={ngo.id} 
                  className={`p-4 rounded-2xl border ${idx === 0 ? 'bg-gradient-to-br from-primary-50 to-primary-100/50 dark:from-primary-900/20 dark:to-transparent border-primary-200 dark:border-primary-800 shadow-md' : 'bg-white/50 dark:bg-gray-800/50 border-gray-200 dark:border-gray-700 hover:bg-white dark:hover:bg-gray-800'}`}
                >
                  {idx === 0 && (
                    <div className="flex items-center gap-1 text-xs font-bold text-primary-600 dark:text-primary-400 mb-2 bg-primary-100 dark:bg-primary-900/40 w-max px-2 py-1 rounded-full uppercase tracking-wider">
                  ...
                  <div className="flex items-center gap-1 mb-2">
                    <div className="flex">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} size={12} className={i < Math.round(ngo.trustScore / 20) ? "text-yellow-400 fill-yellow-400" : "text-gray-300 dark:text-gray-600"} />
                      ))}
                    </div>
                    <span className="text-xs text-gray-500 dark:text-gray-400">({ngo.trustScore} Trust Score)</span>
                  </div>

                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-2 line-clamp-1">Needs: {ngo.needed}</p>
                  ...
                  <button onClick={() => { setSelectedNgo(ngo); setActiveTab('create'); }} className={`w-full mt-3 py-2 rounded-xl text-sm font-bold transition-colors ${idx === 0 ? 'bg-primary-500 text-white hover:bg-primary-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600'}`}>
                    Donate to them
                  </button>
                </motion.div>
              ))}
      ...
      <AnimatePresence>
        {trackingDonation && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            ...
            >
              <LiveTrackingMap 
                donationId={trackingDonation._id}
                pickupLocation={trackingDonation.location}
                dropoffLocation={trackingDonation.acceptedBy?.location}
                isVolunteer={false}
